...
export const S = Object.assign({
  ...
  zapAmt: 210, zapWho: null, snapAmt: 0.01,
  relaysSeen: false,
}, saved);
export function saveUI() {
  store.set("xm.ui.v1", {
    ...
    groupId: S.groupId, channel: S.channel, zapAmt: S.zapAmt, snapAmt: S.snapAmt,
  });
}
...
export function noteCard(e, p, { showTarget = false } = {}) {
  ...
  return `<div class="note-card" data-eid="${e.id}">
    ...
    <div class="note-acts">
      ...
      <button data-act="snap" data-eid="${e.id}" data-pk="${e.pubkey}">${coinMark("xmr", 18)} Snap</button>
    </div>
  </div>`;
}
...
V.welcome = () => `<div class="view opaque immersive" data-view="welcome">
  <div style="flex:1;display:grid;place-items:center;padding:0 34px">
    <div style="text-align:center;width:100%">
      ...
      <h1 class="h1" style="margin-top:24px">Welcome to<br>XM Arcade</h1>
...
V.loginSigner = () => `<div class="view opaque" data-view="loginSigner">
  ...
  <div class="scroller" style="padding:20px 20px 24px">
    ...
    <div class="rows" style="padding:0">
      ${rowItem({ act: "nip07", icon: "key", tint: "var(--primary)", title: "Browser extension", sub: "Use window.nostr (Alby, nos2x…)" })}
    ...
    <div class="card" style="margin-top:14px"><b style="font-size:13px">How pairing works</b>
      <p class="tiny" style="margin:6px 0 0;line-height:1.7">1. Tap <b>Pair a new signer</b> → a <span class="mono">nostrconnect://</span> string appears.<br>2. In Amber: add account → connect → paste it.<br>3. Approve. XM Arcade logs in — no key ever touches the browser.</p></div>
  </div>
</div>`;
...
export function profileHeader(p, stats, mine) {
  ...
  return `<div class="card" style="margin:12px 14px 10px">
    ...
    ${mine ? `<div style="display:flex;gap:8px;margin-top:12px"><button class="btn sm ghost" data-act="sheet" data-sheet="editProfile">Edit profile</button><button class="btn sm ghost" data-act="sheet" data-sheet="keyBackup">Keys</button><button class="btn sm ghost" data-act="go" data-view="compose">Post</button></div>`
      : `<div style="display:flex;gap:8px;margin-top:12px"><button class="btn sm ghost" data-act="zap" data-pk="${p.pubkey}">⚡ Zap</button><button class="btn sm ghost" data-act="snap" data-pk="${p.pubkey}">${coinMark("xmr", 16)} Snap</button></div>`}
  </div>`;
}
V.profile = (ctx = {}) => {
  ...
  return `<div class="short" data-idx="${i}" data-eid="${s.id || ""}">
    ...
    <div class="short-bot">
      ...
      <div class="short-acts">
        ...
        <button class="sact" data-act="reply" data-eid="${s.id || ""}" aria-label="Reply">${ic("reply")}</button>
        <button class="sact" data-act="zap" data-eid="${s.id || ""}" data-pk="${s.pubkey}" aria-label="Zap">${ic("zap")}</button>
        <button class="sact" data-act="snap" data-eid="${s.id || ""}" data-pk="${s.pubkey}" aria-label="Snap (XMR)">${coinMark("xmr", 20)}</button>
    ...
    <div class="scroller" style="padding:4px 14px 12px">
      ...
      ${shown.map((t) => `
        <div class="track-row" data-eid="${t.id}">
          ...
          <button class="icon-btn bare" data-act="snap" data-eid="${t.id}" data-pk="${t.pubkey}" aria-label="Snap (XMR)">${coinMark("xmr", 20)}</button>
