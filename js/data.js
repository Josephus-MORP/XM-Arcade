...
export async function react(target, kind = 1, content = "+", emojiTag = null) {
  const tags = [["e", target.id, "", ""], ["p", target.pubkey], ["k", String(kind)]];
  if (emojiTag) tags.push(emojiTag);
  const e = await signEvent({ kind: 7, content, tags });
  return publish(e);
}
/* ---------- emoji reactions (NIP-25 + NIP-30 packs) ---------- */
const EMOJI_RECENT_KEY = "xm.emoji.recent.v1";
export const recentEmojis = () => store.get(EMOJI_RECENT_KEY, []);
export function pushRecentEmoji(item) {
  const list = [item, ...recentEmojis().filter((x) => x.t !== item.t)].slice(0, 18);
  store.set(EMOJI_RECENT_KEY, list);
}
const packCache = new Map();
export async function getEmojiPacks(pk) {
  if (!pk || pk.startsWith("demo")) return [];
  if (packCache.has(pk)) return packCache.get(pk);
  const p = (async () => {
    const evs = await reqFast({ kinds: [10030], authors: [pk], limit: 5 }, { timeout: 6000 }).catch(() => []);
    const packs = [];
    for (const e of evs) {
      const name = tagVal(e, "d") || "pack";
      const emojis = (e.tags || []).filter((t) => t[0] === "emoji" && t[1] && t[2]).map((t) => ({ code: t[1], url: t[2] }));
      if (emojis.length) packs.push({ name, emojis });
    }
    return packs;
  })();
  packCache.set(pk, p);
...
export async function fetchDiscoveredApps(limit = 30) {
  // Tag discovery + NIP-50 full-text search (nostr.band, Ditto) for app posts.
  const base = reqFast([{ kinds: [1], "#t": ["webxdc"], limit }, { kinds: [1063], "#t": ["webxdc"], limit: 20 }], { timeout: 7000 }).catch(() => []);
  const search = reqFast({ kinds: [1], search: "webxdc", limit: 20 }, { relays: ["wss://relay.nostr.band", "wss://relay.ditto.pub"], timeout: 7000 }).catch(() => []);
  const evs = [...(await base), ...(await search)];
  const seen = new Set();
  const out = [];
  for (const e of evs) {
    if (seen.has(e.id)) continue; seen.add(e.id);
    const urls = [...String(e.content || "").matchAll(/https?:\/\/[^\s)]+/g)].map((m) => m[0]);
    // Blossom-hosted apps use extensionless /<sha256> URLs — accept those too.
    const file = urls.find((u) => /\.(xdc|webxdc|html|zip)(\?|#|$)/i.test(u)) || urls.find((u) => /[0-9a-f]{64}/i.test(u)) || (e.kind === 1063 ? tagVal(e, "url") : "");
    if (!file) continue;
