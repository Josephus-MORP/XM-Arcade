...
import { Signer, isSignedIn, npub, createLocalKey, loginLocalInput, parseBunkerUri, pairingState, loginBunkerUri, pairNewSigner, signEvent, logout, restoreSession } from "./signer.js";
import {
...
import { getEmojiPacks, pushRecentEmoji } from "./data.js";
import {
  Nwc, nwcConnected, connectNwc, disconnectNwc, nwcGetBalance, nwcPayInvoice,
  nwcMakeInvoice, zapProfile, ludToPayUrl, bolt11Msats,
  Xmr, saveXmrCfg, saveXmrMode, xmrStatus, xmrAddress, xmrBalance, xmrTransfer, XMR_ADDR, piconeroToXmr,
} from "./wallets.js";
...
async function fillReactPacks(authorPk) {
  const host = $("#reactPacks");
  if (!host) return;
  try {
    const [mine, theirs] = await Promise.all([getEmojiPacks(Signer.pubkey).catch(() => []), getEmojiPacks(authorPk).catch(() => [])]);
    const el = $("#reactPacks");
    if (!el) return;
    const eid = esc(el.dataset.eid || "");
    const packs = [...mine.map((p) => ({ ...p, who: "Your packs" })), ...theirs.map((p) => ({ ...p, who: "Their packs" }))];
    el.innerHTML = packs.length ? packs.map((p) => `<div class="react-pack"><div class="tiny" style="font-weight:800;margin:10px 0 6px">${esc(p.who)} · ${esc(p.name)}</div><div class="react-grid">${p.emojis.slice(0, 24).map((e) => `<button class="react-cell" data-act="reactGo" data-eid="${eid}" data-emoji=":${esc(e.code)}:" data-eurl="${esc(e.url)}" title=":${esc(e.code)}:"><img src="${esc(e.url)}" alt=":${esc(e.code)}:" loading="lazy"></button>`).join("")}</div></div>`).join("") : `<p class="tiny" style="margin-top:10px">No emoji packs on either profile yet.</p>`;
...
async function loadWallet() {
  ...
  if (Xmr.mode === "external") { ctx.xmr = { ok: true, external: true }; }
  else try {
    await xmrStatus();
...
async function afterSheetOpen(id, ctx, wrap) {
  if (id === "feedApp" || id === "feedPick") {
    ...
    if (!GroupsCache.length) GroupsCache = [...(await fetchGroups())];
  ...
  actGuard(async () => {
    ...
    if (a === "pairNew") {
      ...
      $("#signerPane").innerHTML = `<div class="card"><b style="font-size:13px">Approve in Amber</b><div class="keybox" style="margin:8px 0;font-size:10.5px">${esc(pair.uri)}</div>
        <div style="display:flex;gap:8px"><button class="btn sm" data-act="copyPair">Copy</button><button class="btn sm" data-act="pairOpen">Open in Amber</button><button class="btn sm line" data-act="pairCancel">Cancel</button></div>
    ...
    if (a === "pairOpen") { const u = window.__pair && window.__pair.uri; if (!u) { toast("Pair first", "info"); return; } window.location.href = u; toast("Opening your signer…", "key"); return; }
    ...
    if (a === "reply") {
      ...
      if (d.inline) {
        const card = t.closest ? t.closest(".short") : null;
        if (!card) { toast("Couldn't place the reply box", "info"); return; }
        card.querySelector(":scope > .inline-reply")?.remove();
        const box = document.createElement("div");
        box.className = "inline-reply";
        box.innerHTML = `<textarea id="inlineReplyText" placeholder="Reply…" rows="2"></textarea>
          <div class="inline-reply-bar"><button class="btn sm" data-act="replyGoInline" data-eid="${esc(target.id)}" data-pk="${esc(target.pubkey)}" data-kind="${target.kind ?? 1}">Post</button>
          <button class="btn sm ghost" data-act="replyCancelInline">Cancel</button></div>`;
    ...
    if (a === "replyCancelInline") { const box = t.closest ? t.closest(".inline-reply") : null; if (box) box.remove(); return; }
    if (a === "replyGoInline") {
      const box = t.closest ? t.closest(".inline-reply") : null;
      const ta = box ? box.querySelector("textarea") : null;
      const body = (ta?.value || "").trim();
      if (!body) { toast("Write something first", "info"); return; }
      t.disabled = true;
      const tags = [["e", d.eid, "", "reply"], ["p", d.pk || ""], ["k", String(d.kind ?? 1)]];
      const { res } = await publishNote(body, tags);
      if (box) box.remove();
      toast(res.ok ? `Replied on ${res.oks}/${res.total} relays` : "No relay accepted it", res.ok ? "check" : "info"); return;
    }
    if (a === "repost") {
      ...
      const r = await repost(evs[0]);
      toast(r.ok ? "Reposted" : "No relay accepted it", r.ok ? "refresh" : "info"); return;
    }
    if (a === "react") {
      if (!isSignedIn()) { toast("Sign in first", "info"); return; }
      const id = d.eid;
      ...
      const evs = await reqFast({ ids: [id] });
      if (!evs[0]) { toast("Not found on relays", "info"); return; }
      openSheet("react", { eid: evs[0].id, kind: evs[0].kind, pk: evs[0].pubkey });
      fillReactPacks(evs[0].pubkey);
      return;
    }
    if (a === "reactGo") {
      const emoji = d.emoji || val("reactCustom").trim();
      if (!emoji) { toast("Pick or type an emoji", "info"); return; }
      const evs = await reqFast({ ids: [d.eid] });
      if (!evs[0]) { toast("Not found on relays", "info"); return; }
      pushRecentEmoji(d.eurl ? { t: emoji, u: d.eurl } : { t: emoji });
      const tag = d.eurl ? ["emoji", emoji.replace(/:/g, ""), d.eurl] : null;
      const r = await react(evs[0], evs[0].kind, emoji, tag);
      closeSheet(true);
      toast(r.ok ? "Reacted" : "No relay accepted it", r.ok ? "heart" : "info"); return;
    }
    ...
    if (a === "videoToggle") {
      const card = t.closest ? (t.closest(".short") || null) : null;
      const v = t.tagName === "VIDEO" ? t : t.querySelector("video");
      if (v) {
        v.muted = false;
        const willPlay = v.paused;
        willPlay ? v.play().catch(() => {}) : v.pause();
        if (card) {
          let f = card.querySelector(":scope > .tap-flash");
          if (!f) { f = document.createElement("div"); f.className = "tap-flash"; card.appendChild(f); }
          f.innerHTML = `<span>${ic(willPlay ? "play" : "pause")}</span>`;
    ...
    /* ---- zaps ---- */
    if (a === "zap") { if (!nwcConnected()) { openSheet("needWallet", { kind: "sats" }); return; } openSheet("zap", { eid: d.eid || "", pk: d.pk || "" }); return; }
    ...
    if (a === "xap") {
      if (Xmr.mode !== "external") {
        try { await xmrStatus(); Xmr.mode = "rpc"; saveXmrMode(); }
        catch { openSheet("needWallet", { kind: "xmr" }); return; }
      }
      openSheet("xap", { eid: d.eid || "", pk: d.pk || "" }); return;
    }
    if (a === "xapAmt") { S.xapAmt = Number(d.val); saveUI(); openSheet("xap", { eid: d.eid || "", pk: d.pk || "" }); return; }
    if (a === "xapGo") {
      const to = val("xapTo").trim();
      if (!XMR_ADDR.test(to)) { toast("Paste their Monero address (starts with 4)", "info"); return; }
      const custom = Number(val("xapCustom") || 0);
      const amt = custom > 0 ? custom : S.xapAmt;
      if (!(amt > 0)) { toast("Enter an amount", "info"); return; }
      if (Xmr.mode === "external") {
        closeSheet(true);
        window.location.href = `monero:${to}?tx_amount=${amt}`;
        toast("Opening your wallet…", "xmr"); return;
      }
      t.disabled = true; t.textContent = "Xapping…";
      const r = await xmrTransfer(to, amt);
      const h = r.tx_hash ? ` · ${r.tx_hash.slice(0, 8)}` : "";
      closeSheet(true); toast(`Xapped ${amt} XMR${h}`, "xmr"); return;
    }
    ...
    if (a === "needWalletGo") { closeSheet(true); go("wallet", {}); return; }
    if (a === "xmrExternalGo") { Xmr.mode = "external"; saveXmrMode(); closeSheet(true); loadWallet(); toast("External wallet set", "xmr"); return; }
    if (a === "xmrExtGo") {
      const to = val("xmrExtTo").trim(), amt = Number(val("xmrExtAmt") || 0);
      if (!XMR_ADDR.test(to)) { toast("That Monero address looks wrong", "info"); return; }
      if (!(amt > 0)) { toast("Enter an amount", "info"); return; }
      closeSheet(true); window.location.href = `monero:${to}?tx_amount=${amt}`; toast("Opening your wallet…", "xmr"); return;
    }
    if (a === "localGo") { connectNwc(val("localNwc")); closeSheet(true); toast("Local wallet connected", "zap"); loadWallet(); return; }
...
document.addEventListener("input", (e) => {
  if (e.target.matches?.(".accentHue")) { S.accentHue = Number(e.target.value); saveUI(); applyAccent(); }
});
...
function applyAccent() {
  // Secondary color: user hue slider. Default 187 == the classic blue (#22d3ee).
  const h = S.accentHue ?? 187, dark = (S.theme || "dark") === "dark";
  document.documentElement.style.setProperty("--accent", `hsl(${h},85%,${dark ? 55 : 40}%)`);
  $$(".accentHue").forEach((r) => { r.value = h; });
}
function fit() {
