  ...
  return `<div class="view opaque chats-view" data-view="chats">
    ...
    <div style="display:flex;flex:1;min-height:0">
      ...
      <div class="scroller" style="flex:1;padding:12px 14px">
        ...
        : emptyState("No groups", ctx.loading ? "Asking group relays…" : "Add a group relay in Settings → Relays, or browse the demo.", `<button class="btn sm" style="margin-top:14px" data-act="sheet" data-sheet="relays">Relay settings</button>`)}
      </div>
    </div>
  </div>`;
};
...
function satsHalf(ctx) {
  ...
  return `<section class="wallet-half ${ready ? "wallet-ready" : ""}" data-wallet="sats">
    ...
    ${!ready ? `<div class="wallet-welcome-text"><h3>Welcome to sats</h3><p>Connect your own Lightning wallet with Nostr Wallet Connect — funds stay there.</p></div>
      <div class="wallet-actions"><button class="btn block wallet-primary" data-act="sheet" data-sheet="nwc">${ic("nwcLink")} Connect NWC wallet</button></div>`
...
function xmrHalf(ctx) {
  ...
  return `<section class="wallet-half ${ready ? "wallet-ready" : ""}" data-wallet="xmr">
    <div class="wallet-half-head">${coinMark("xmr")}
      ...
      <span class="wallet-status ${ready ? "is-ready" : ""}">${ready ? "RPC live" : "Not set up"}</span></div>
    ${!ready ? `<div class="wallet-welcome-text"><h3>Welcome to Monero</h3><p>Point XM Arcade at <b>your own</b> monero-wallet-rpc. No seeds touch this app.</p></div>
      <div class="wallet-actions"><button class="btn block wallet-primary" data-act="sheet" data-sheet="xmrCfg">${ic("plus")} Connect wallet RPC</button></div>
...
SHEETS.author = (ctx = {}) => {
  ...
  return {
    ...
    <div class="rows" style="padding:0">
      ...
      ${rowItem({ act: "snap", pk, icon: "xmr", tint: "#ed4357", title: "Snap", sub: "Send XMR via wallet RPC", chev: false })}
...
SHEETS.snap = (ctx = {}) => ({
  title: "Monero snap", sub: ctx.eid ? "for this post" : "to this user",
  body: `<div class="zap-grid">${[0.001, 0.01, 0.05, 0.1].map((a) => `<button class="zap-amt snap-amt ${S.snapAmt === a ? "on" : ""}" data-act="snapAmt" data-val="${a}" data-eid="${esc(ctx.eid || "")}" data-pk="${esc(ctx.pk || "")}">${coinMark("xmr", 15)} ${a}<small>XMR</small></button>`).join("")}</div>
    <input class="field" id="snapCustom" inputmode="decimal" placeholder="Custom XMR amount">
    <label class="tiny" style="display:block;margin:10px 0 4px">Their Monero address</label>
    <input class="field mono" id="snapTo" placeholder="4…" autocapitalize="off" spellcheck="false">
    <button class="btn block" style="margin-top:12px" data-act="snapGo" data-eid="${esc(ctx.eid || "")}" data-pk="${esc(ctx.pk || "")}">${coinMark("xmr", 18)} Snap it</button>
    <p class="tiny" style="margin-top:10px;text-align:center">Pays from your wallet RPC (Wallet → Monero). Profiles don\u2019t carry XMR addresses yet — ask them for theirs. No public receipt: the amount stays private.</p>`,
});
SHEETS.editProfile = () => {
...
SHEETS.about = () => ({
  ...
  body: `<p class="sub" style="line-height:1.7">Shorts (NIP-71) · Concord groups (NIP-29 + NIP-44/59) · sats via NWC + zaps (NIP-47/57) · Monero via wallet RPC · music · webxdc arcade.<br><br>Signers: device key, NIP-07 extension, or NIP-46 remote (Amber). Crypto: vendored nostr-tools (MIT, paulmillr.com) — works offline, relays excluded.</p>`,
});
